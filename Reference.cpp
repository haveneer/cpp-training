...
int main() { // Error messages come from GCC 9.2
  ...
  *p = 666;    // OK: lvalue = rvalue
  ...
  std::cout << "BEFORE i=" << i << ", a=" << a << ", b=" << b << ", c=" << c << ", d=" << d << std::endl;
  set_ref(++i);
  set_ref(a = b);
  set_ref((c, d));
  std::cout << "AFTER  i=" << i << ", a=" << a << ", b=" << b << ", c=" << c << ", d=" << d << std::endl;
  // set_ref(i++); // error: cannot bind non-const lvalue reference of type
  //                    // 'int&' to an rvalue of type 'int'

  set_rvalue_ref(42);
  // set_rvalue_ref(i); // error: cannot bind rvalue reference of type 'int&&' to lvalue of type 'int'

  // int & q = rvalue_ref(e); // error: cannot bind rvalue reference of type 'int&&' to lvalue of type 'int'
