...
void display(const std::map<char, int> &counts, const int min_occur = 30) {
  std::cout << "-------------\n";
  for (auto &&kv : counts) {
    auto &&key = kv.first;
  ...
  for (char key = 'a'; key <= 'z'; ++key) {
    // int value = counts[key]; // error: passing 'const std::map<char, int>' as 'this' argument discards qualifiers
    try {
