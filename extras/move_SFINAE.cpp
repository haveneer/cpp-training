...
struct X {
  ...
  X() : id(++cnt), owner(true) { std::cout << "X() #" << id << std::endl; }

  // non-const lvalue - copy ctor
  X(X &rhs) : id(++cnt), owner(true) {
    std::cout << "copy #" << id << " <- #" << rhs.id << std::endl;
  }

  // const lvalue - T will be deduced as X const
  template <class T>
  X(T &rhs, typename enable_if_same<X const, T>::type = 0) : id(++cnt), owner(true) {
    std::cout << "copy #" << id << " <- #" << rhs.id << " (const)" << std::endl;
  }

  ~X() { std::cout << "destroy #" << id << (owner ? "" : " (EMPTY)") << std::endl; }

  X &operator=(X rhs) {
    std::cout << "ASSIGN #" << id << (owner ? "" : " (EMPTY)") << " <== #" << rhs.id
              << (rhs.owner ? "" : " (EMPTY)") << std::endl;
    owner = rhs.owner;
    rhs.owner = false;
    assert(owner);
    return *this;
...
public: // Move stuff
  ...
  // non-const rvalue
  X(ref rhs) : id(++cnt), owner(rhs.p->owner) {
...
X const csource() { return X(); }

void sink(X) { std::cout << "in rvalue sink" << std::endl; }

void sink2(X &) { std::cout << "in non-const lvalue sink2" << std::endl; }

void sink2(X const &) { std::cout << "in const lvalue sink2" << std::endl; }

void sink3(X &) { std::cout << "in non-const lvalue sink3" << std::endl; }
...
int main() {
  ...
  std::cout << " ------ test 5, check const ------- " << std::endl;
  X const z7;
  ...
  std::cout << " ------ test 10, pass rvalue by overloaded reference ------- "
            << std::endl;
  // This one fails in Comeau's strict mode due to 8.5.3/5.  GCC 3.3.1 passes it.
  sink2(source());

  std::cout << " ------ test 11, pass const rvalue by overloaded reference ------- "
            << std::endl;
  sink2(csource());
  ...
  std::cout << " ------ test 17, pass const lvalue by const reference ------- "
            << std::endl;
  sink2(z7);

  std::cout << " ------ test 18, pass const lvalue by-reference ------- "
            << std::endl;
#if 0 // correctly fails to compile, just as desired
    sink3(z7);
#endif

  std::cout << " ------ test 19, pass rvalue by value to template param ------- "
            << std::endl;
  tsink(source());

  std::cout << " ------ test 20, direct initialize a const A with an A ------- "
            << std::endl;
  typedef X const XC;
  sink2(XC(X()));
